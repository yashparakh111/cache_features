...
trace_features_format_t curr_features;
...
KNOB<string> KnobCallStackOutputFile(KNOB_MODE_WRITEONCE,  "pintool", "O", "champsim_features.trace", 
...
void BeginInstruction(VOID *ip, UINT32 op_code, VOID *opstring)
{
	...
	curr_features.ip = (unsigned long long int)ip;
	curr_features.call_stack_size = 0;
	curr_features.loop_depth = 0;

	for(int i = 0; i < call_stack_max_size; i++) {
		curr_features.call_stack[i] = 0;
	}
}
...
void EndInstruction() {
	...
	if(instrCount > KnobSkipInstructions.Value())
	{
		...
		if(instrCount <= (KnobTraceInstructions.Value()+KnobSkipInstructions.Value()))
		{
			...
			if(is_read) {
				fwrite(&curr_features, sizeof(trace_features_format_t), 1, features_output_file);
			}
		}
		else
		{
...
void MemoryRead(INS instr, VOID* addr, UINT32 index, UINT32 read_size) {
	...
	if(!call_stack_address.empty()) {
		...
		if(RTN_Valid(curr_rtn) && SYM_Valid(RTN_Sym(curr_rtn))) {
			...
			int offset = curr_features.ip - (unsigned long long int)rtn_addr;
	...
	for(int i = 0; i < curr_features.call_stack_size; i++) {
		curr_features.call_stack[i] = *rtn_addr_it;
		rtn_addr_it++;
	}
}
